...
describe ('Character', function () {
  it('takeDamage - Character should recieve damage and update health based on damage taken', function() {
    let character = new Character(10, 5, "Hero");

    character.takeDamage(2);

    expect(character.health).to.equal(8);
  });

  it('takeDamage + die - Character should die if damage takes health below 1', function() {
    let character = new Character(10, 5, "Hero");

    character.takeDamage(11);

    expect(character.health).to.equal(-1);
    expect(character.isDead).to.equal(true);
  });

  it('attack - Character do damage to other character type', function() {
    let character = new Character(10, 5, "Hero");
    let characterTwo = new Character(10, 2, "Enemy");

    character.attack(characterTwo);

    expect(characterTwo.health).to.equal(5);
  });
});